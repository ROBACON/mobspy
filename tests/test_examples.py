...
class TestSimpleRepressor:
    def test_run(self):
        ...
        Promoter >> (Promoter + Protein) @ (
            lambda promoter: 1 if promoter.inactive else 0
        )
        Protein >> Zero @ 0.1
...
class TestANDGate:
    def test_run(self):
        ...
        Promoter >> (Promoter + Protein) @ (
            lambda promoter: 1 if promoter.active else 0
        )
        Protein >> Zero @ 2
...
class TestSimpleRuleBasedANDGate:
    def test_compile(self):
        ...
        def Promoter_Rule(Promoter, Ligand, Protein, K):
            (
                Promoter + Ligand
                >> (Promoter + Ligand + Protein)
                @ (lambda p, loc: (p / u.h) * loc**4 / (K**4 + loc**4))
            )
...
class TestBioCRNpyler2:
    def test_compile(self):
        ...
        def Read(Pro, R, strand):
            ...
            (
                Pro + R.c("free_" + str(R))
                >> (Pro + R.c(sp).c("at_" + strand[0][0])) @ rate
            )
...
class TestKappaComparison:
    def test_compile(self):
        ...
        A.nl_1 + B.nl_1 >> (A.l_1 + B.l_1) @ (1e-4, lambda r: 0.1 * r)
...
class TestPhageTransmissionSystem:
    def test_compile(self):
        ...
        (
            Receiver.old + Resource
            >> (Receiver.young + Receiver.not_infected.young) @ grw_r
        )
