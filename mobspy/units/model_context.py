...
class ModelUnitContext:
    ...
    volume_unit: Unit = field(default_factory=lambda: _ur.liter)
    dimension: int = 3
    ...
    def convert_rate(  # noqa: PLR0911  # complex DSL method with multiple return paths
        self,
    ...
    def convert_counts(  # noqa: PLR0911  # many returns
        self,
    ...
    def convert_volume(self, volume: int | float | Quantity) -> int | float:
        ...
        return volume  # pyright: ignore[reportReturnType]  # pint Quantity subtype

    def convert_time(self, time: int | float | Quantity) -> int | float | None:
        ...
        if isinstance(time, Quantity):
            ...
            return None
        return time  # pyright: ignore[reportReturnType]  # pint Quantity subtype
...
def _extract_substance_unit(quantity: Quantity) -> Unit:
    ...
    for target in candidates:
        try:
            ...
            if _UNIT_MATCH_LOWER < abs(converted.magnitude) < _UNIT_MATCH_UPPER:
                return target  # type: ignore[no-any-return]  # dynamic dispatch
...
def _extract_volume_unit(quantity: Quantity, dimension: int) -> Unit:
    ...
    return _ur.decimeter**dimension  # type: ignore[return-value]  # pint Unit subtype
...
def _create_unit_def(model: Any, unit_id: str, pint_unit: Unit) -> None:
    ...
    if components is None:
        # Fallback: dimensionless  # noqa: ERA001  # kept as documentation reference
...
def _decompose_pint_unit(pint_unit: Unit) -> list[_SbmlUnitComponent] | None:
    ...
    try:
        ...
        q = (1 * pint_unit).to_base_units()  # pyright: ignore[reportAttributeAccessIssue]  # pint method
