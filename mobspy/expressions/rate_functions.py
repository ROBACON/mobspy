...
from mobspy.dsl.species_constructors import Zero as mc_Zero
...
from mobspy.units.handler import convert_rate as uh_convert_rate
...
def extract_reaction_rate(  # noqa: PLR0913  # complex function signature
...
def _process_callable_rate(  # noqa: PLR0913  # complex function signature
    combination_of_reactant_species: list[dict[str, Any]],
    reactant_string_list: list[str],
    reaction_rate_function: _RateFunction,
...
def prepare_arguments_for_callable(
    ...
    if rate_function_arguments is not None:
        ...
        for i, (species, reactant_string) in enumerate(
            ...
            try:
                species = species["object"]  # noqa: PLW2901  # intentional
        ...
        fill_start = len(argument_dict)
        for idx in range(fill_start, len(rate_function_arguments)):
            argument_dict[rate_function_arguments[idx]] = mbe_Specific_Species_Operator(
                NULL_SPECIES, mc_Zero
            )
