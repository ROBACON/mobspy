...
def convert_data_to_desired_unit(  # noqa: PLR0913  # complex function signature
    ...
    if unit_x is not None:
        ...
        converted_data["Time"] = [
            (time * time_unit).to(unit_x).magnitude  # pyright: ignore[reportAttributeAccessIssue]  # pint method
...
def convert_to_concentration(
    ...
    for i in range(len(data["Time"])):
        if (
            k < len(time_list)
            and data["Time"][i] > time_list[k]
            and k + 1 < len(volume_list)
        ):
            k = k + 1
