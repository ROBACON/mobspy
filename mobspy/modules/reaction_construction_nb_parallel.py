...
def iterator_for_combinations(list_of_lists):
    ...
    """
    return [i for i in itertools.product(*list_of_lists)]
...
def create_all_reactions(reactions, meta_species_in_model,
    ...
    a = Parallel(n_jobs=-1, prefer="threads")(delayed(get_involved_species)(reaction, meta_species_in_model)
                                              for reaction in reactions)
    prod = Parallel(n_jobs=-1, prefer="threads")(delayed(construct_product_structure)
                                                 (reaction) for reaction in reactions)

    bso = [e[0] for e in a]
    rscl = [e[1] for e in a]

    crs = Parallel(n_jobs=-1, prefer="threads")(delayed(iterator_for_combinations)(rs) for rs in rscl)

    crs = [(c[0], b, p) for c, b, p in zip(crs, prod, bso)]

    rcomb_string = Parallel(n_jobs=-1, prefer="threads")(delayed(construct_reactant_structures)
                                                         (x[0], ref_characteristics_to_object) for x in crs)

    rsl = Parallel(n_jobs=-1, prefer="threads")(delayed(iterator_for_combinations)(r) for r in rcomb_string)
    rsl = [(r[0], x[1], x[2]) for r, x in zip(rsl, crs)]

    ord_str = Parallel(n_jobs=-1, prefer="threads")(delayed(construct_order_structure)(r[2], r[0]) for
                                                    r in rsl)
    print(ord_str)
    exit()
    ...
    # for r in rcomb_string:
    #    print(r)
    # exit()

    # pol = Parallel(n_jobs=-1, prefer="threads")(delayed(construct_product_structure)(reaction) for
