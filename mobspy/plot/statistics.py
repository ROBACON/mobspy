...
def time_series_average(species_string: str, mobspy_ts: Any) -> list[float]:
    ...
    for j in range(len(mobspy_ts.get_max_time_for_species(species_string))):
        ...
        average_series.append(add / size if size > 0 else 0.0)
...
def standard_deviation(
    ...
    for j in range(len(mobspy_ts.get_max_time_for_species(species_string))):
        ...
        deviation_series.append(np.sqrt(add / size) if size > 0 else 0.0)
...
def average_plus_standard_deviation(
    ...
    plus: list[float] = []
    minus: list[float] = []
    for average, deviation in zip(series_average, series_deviation, strict=True):
