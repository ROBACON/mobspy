...
from mobspy.units.handler import convert_counts as uh_convert_counts
from mobspy.units.handler import convert_time as uh_convert_time
...
def format_event_dictionary_for_sbml(  # noqa: PLR0913  # complex function signature
...
def _process_all_char_assignments(
    ...
    assert isinstance(ev.event_counts, list)  # noqa: S101  # type narrowing assert
...
def _assemble_events_for_sbml(
    ...
    for i, event in enumerate(reformed_event_list):
        assert isinstance(event.event_counts, dict)  # noqa: S101  # type narrowing
