...
from mobspy.dsl.assignments_implementation import Assign
...
def generate_ODE_reaction_rate(list_of_used_species: list[Any], expression: Any) -> Any:  # noqa: N802  # legacy name
...
class ODEBinding:
    ...
    def _process_ode_expression(
        ...
        if is_birth:
            _ = reactants >> (self.state_variable + reactants) @ rate_fn  # type: ignore[operator]  # DSL dynamic type
        else:
            _ = (reactants + self.state_variable) >> reactants @ rate_fn  # type: ignore[operator]  # DSL dynamic type
