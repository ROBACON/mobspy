...
from mobspy.dsl.reactions import _Last_rate_storage
from mobspy.dsl.species import Species, clean_species_name
...
def compile_species_number_line(code_line: str) -> tuple[int, list[str]]:
    ...
    _parts = code_line.split("=", maxsplit=1)
    before_eq, after_eq = _parts[0], _parts[1]
...
def _Create_Species(  # noqa: N802  # legacy DSL method name
    ...
    for i in range(number_of_properties):
        ...
        if species is None:
            ...
            temp = One * species
            temp.name(name)  # type: ignore[operator]  # DSL dynamic type
            to_return.append(temp)  # type: ignore[arg-type]  # DSL flexibility
...
def BaseSpecies(  # noqa: N802  # legacy DSL method name
    number_or_names: int | list[str] | None = None,
) -> Species | tuple[Species, ...]:
...
def New(  # noqa: N802  # legacy DSL method name
    species: Species,
    number_or_names: int | list[str] | None = None,
) -> Species | tuple[Species, ...]:
...
def ListSpecies(  # noqa: N802  # legacy DSL method name
    ...
    return List_Species(temp_list)  # type: ignore[arg-type]  # DSL flexibility
