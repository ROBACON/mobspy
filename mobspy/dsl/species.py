...
from mobspy.dsl.declarations import CountAssignment, RatedProduct, get_registry
...
from mobspy.dsl.species_operators import (
    Specific_Species_Operator as me_Specific_Species_Operator,
)
from mobspy.dsl.species_utils import (
    check_orthogonality_between_references as mcu_check_orthogonality_between_references,  # noqa: E501  # long import path
)
from mobspy.dsl.species_utils import (
...
_FRAMEWORK_ATTRS: frozenset[str] = frozenset(
    {"_pytestfixturefunction", "__sphinx_mock__"}
)
...
def _create_reaction_from_rated(
    ...
    """
    from mobspy.units.validation import validate_rate_units  # noqa: PLC0415
...
class Species(lop_SpeciesComparator, Assignment_Opp_Imp):
    ...
    def check_if_valid_characteristic(
        ...
        if (
            char[0] == "_"
            and char not in _FRAMEWORK_ATTRS
    ...
    def str_under_context(
        ...
        """
        ref_char_to_spe_obj = (
            Species.get_simulation_context().orthogonal_vector_structure  # type: ignore[union-attr]  # guarded at runtime
        )
        all_strings = sorted(
    ...
    def __or__(self, other: Species | List_Species) -> List_Species:
        ...
        Args:
            other: To combine.
        """
        from mobspy.dsl.list_species import List_Species  # noqa: PLC0415
    ...
    def __getitem__(self, item: Any) -> Self:
        ...
        Args:
            item: Reaction rate.
        """
        return _Last_rate_storage.override_get_item(self, item)  # type: ignore[return-value,arg-type]  # pint Unit subtype
    ...
    def __rmul__(  # type: ignore[override]  # DSL overload
    ...
    def _resolve_quantity(
        ...
        if len(_any_ctx) != 0:
            for i in _any_ctx:
                self.c(i)
            return self.add_quantities(_any_ctx.copy(), quantity)  # type: ignore[arg-type]  # DSL flexibility
    ...
    def _apply_event_context(
        ...
        try:
            sim_under_context.current_event_count_data.append(  # type: ignore[union-attr]  # guarded at runtime
                {
                    "species": self,
        ...
        if self.get_simulation_context() is None:
            ...
            get_registry().add_count(
                CountAssignment(
                    ...
                    quantity=quantity,  # type: ignore[arg-type]  # DSL flexibility
                )
            )
        else:
    ...
    def __mul__(self, other: Species) -> Species | MobsPyExpression:  # type: ignore[override]  # DSL overload
    ...
    def _mul_impl(self, other: Species, name: str) -> Species:
        ...
        new_entity._from_mul = True  # type: ignore[attr-defined]  # mixin attr
...
_methods_Species = set(dir(Species))  # noqa: N816  # legacy DSL variable convention
