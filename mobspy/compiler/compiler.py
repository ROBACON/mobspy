...
from mobspy.compiler.expansion import (
...
from mobspy.dsl.event_functions import (
...
from mobspy.dsl.species_constructors import EndFlagSpecies
...
def phase_parameter_validation(
    ...
    all_param_objects: set[mp_Mobspy_Parameter] | frozenset[mp_Mobspy_Parameter],
) -> None:
    ...
    seen_names: dict[str, mp_Mobspy_Parameter] = {}
    for param in all_param_objects:
        if param.name in seen_names and seen_names[param.name] is not param:
            raise CompilationError(
                "There are two different Parameter Objects with the same name"
            )
        seen_names[param.name] = param
...
def _add_to_parameters_to_sbml(
    ...
    parameters_to_add: set[mp_Mobspy_Parameter] | frozenset[mp_Mobspy_Parameter],
    model_context: ModelUnitContext | None = None,
) -> None:
    ...
    for parameter in parameters_to_add:
        ...
        val = (vals[0] if vals else 0.0) if isinstance(vals, list) else vals
        parameters_for_sbml[parameter.name] = (val, param_unit)
...
def _resolve_volume_and_dimension(
    ...
    for count in species_counts:
        if isinstance(count["quantity"], Quantity):
            extracted = uh_extract_length_dimension(
...
def _apply_count_to_species(
    ...
    if isinstance(quantity, mp_Mobspy_Parameter):
        ...
        if quantity.name in acc.parameters_used:
            acc.parameters_used[quantity.name].used_in.update(species_strings)
        else:
...
def _build_events(
    ...
    if ctx.continuous_sim:
        if ctx.ending_condition is None:
            raise CompilationError(
                "ending_condition is required for continuous simulations"
            )
        end_event = EventData(
...
def _generate_model_string(  # noqa: PLR0913  # complex function signature
    species_for_sbml: SpeciesForSbml,
    mappings_for_sbml: MappingsForSbml,
    parameters_for_sbml: ParametersForSbml,
    reactions_for_sbml: ReactionsForSbml,
...
def compile_model(  # noqa: PLR0913  # complex function signature
    meta_species_to_simulate: List_Species,
    reactions_set: set[Any],
    species_counts: list[dict[str, Any]],
    orthogonal_vector_structure: dict[str, Any],
    *,
    ...
    parameter_object_dict = {
        key: parameter_exist[key] for key in parameters_used if key in parameter_exist
    }
    ...
    concrete = ConcreteModel(
        species=SpeciesDict(species_for_sbml),
